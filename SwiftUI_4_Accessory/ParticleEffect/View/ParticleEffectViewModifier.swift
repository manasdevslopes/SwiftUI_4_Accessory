//
//  ParticleEffectViewModifier.swift
//  SwiftUI_4_Accessory
//
...
extension View {
  @ViewBuilder
  func ParticleEffectViewModifier(systemImage: String, font: Font, status: Bool, activeTint: Color, inActiveTint: Color) -> some View {
    self
      .modifier(
...
fileprivate struct ParticleModifier: ViewModifier {
  ...
  /// View properties
  @State private var particles: [ParticleEffectModel] = []
      ...
      .overlay(alignment: .top) {
        ZStack {
          ForEach(particles) { particle in
            Image(systemName: systemImage).foregroundColor(status ? activeTint : inActiveTint).scaleEffect(particle.scale)
              .offset(x: particle.randomX, y: particle.randomY).opacity(particle.opacity)
        ...
        .onAppear {
          ...
          if particles.isEmpty {
            for _ in 1...15 {
              let particle = ParticleEffectModel()
